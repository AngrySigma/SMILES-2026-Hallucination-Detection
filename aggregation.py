...
Each block carries its own ``(name, extractor, dim)``. Sub-probes in
``probe.py`` reference blocks by name and read contiguous slices via
``BLOCK_SLICES[name]``.
...
#
# Built once per process — at module load if any active sub-probe references
# regen_features — so the builder's Qwen instance and solution.py's Qwen
# never co-reside on GPU.
# ---------------------------------------------------------------------------

def _pick_regen_batch_size() -> int:
    """Pick a generation batch size from available GPU memory.

    Sized to the observed peak memory per sample (≈100 MB of KV cache +
    activations for this dataset's prompts at 256 generated tokens). The
    OOM-safe fallback in ``_run_regen_chunk`` recovers per-sample if a
    chunk happens to exceed memory, so picking aggressively is cheap.
    """
    if not torch.cuda.is_available():
        return 1
    vram_gb = torch.cuda.get_device_properties(0).total_memory / 1e9
    if vram_gb > 60:    # H100 80GB, A100 80GB
        return 128
    if vram_gb > 30:    # A100 40GB
        return 128
    if vram_gb > 12:    # T4 16GB
        return 256
    return 1


class _Top1LogprobCapture(LogitsProcessor):
    """Captures the per-step top-1 logprob on CPU, so generate() can free
    its full ``(batch, max_new, vocab)`` scores tensor (≈10 GB at batch 128).
...
def _compute_regen_stats_from_ids(
    ...
    """Build the 6-d regen feature row from precomputed token sequences:
        0: token Jaccard
        1: token overlap ratio (|inter| / |actual|)
        2: first-N-token positional match rate
        3: regen length / actual length
        4: regen length − actual length
        5: mean per-token logprob of the greedy regen
    """
    if not gen_ids:
    ...
    if n > 0:
        first_n_match = sum(
            1 for i in range(n) if gen_ids[i] == actual_ids_list[i]
        ) / n
    else:
        first_n_match = 0.0
    ...
    return np.array([
        jaccard, overlap_ratio, first_n_match, length_ratio, length_diff,
        avg_logprob,
    ], dtype=np.float32)
...
def _run_regen_chunk(
    ...
    Pads prompts left to the chunk's max length — caller should sort chunks
    by prompt length to minimise padding waste. Writes the 6-d feature row
    into ``feats[i]`` for each ``i`` in ``sample_indices``.
    """
    ...
    attn_list = [
        [0] * (max_prompt_len - len(all_prompt_ids[i]))
        + [1] * len(all_prompt_ids[i])
        for i in valid
    ]
    ...
    if capture.logprobs:
        per_step_lp = torch.stack(capture.logprobs, dim=0)  # (n_steps, batch)
    else:
    ...
    for k, i in enumerate(valid):
        ...
        if eos_id is not None and eos_id in seq_k:
            end = seq_k.index(eos_id) + 1  # inclusive of eos
            gen_ids = seq_k[:end]
        else:
            gen_ids = seq_k
        n_real = len(gen_ids)
        ...
        feats[i] = _compute_regen_stats_from_ids(
            gen_ids, all_actual_ids[i], avg_lp, first_n,
        )
...
def _build_regen_cache_inline() -> None:
    ...
    continuation for every (prompt, response) row in dataset.csv + test.csv,
    then persist it to ``regenerations.npz``.

    Loads its own Qwen instance at bf16, runs batched generation
    (length-sorted to minimise padding waste), frees the model. Runtime is
    dominated by ``model.generate()`` — ≈25 min on T4, ≈30 s on H100.
    """
    global _REGEN_CACHE
    ...
    model = AutoModelForCausalLM.from_pretrained(
        _DEFAULT_MODEL, torch_dtype=torch.bfloat16,
    )
    ...
    # Length-sort so each batch has similar-length prompts and padding is
    # minimised; original index order is restored via per-sample writes.
    ...
    while chunk_idx * batch_size < n_total:
        ...
        try:
            _run_regen_chunk(
                model, tok, device, chunk,
                all_prompt_ids, all_actual_ids, feats,
            )
            ...
            for i in chunk:
                try:
                    ...
                    print(
                        f"[aggregation]   sample {i}: {type(e).__name__}: {e}, "
                        "zeros",
                        flush=True,
                    )
                    feats[i] = np.zeros(6, dtype=np.float32)
...
def response_mask(attention_mask: torch.Tensor) -> torch.Tensor:
    """Return the boolean mask over response tokens AND refresh the
    per-sample ``_CURRENT_INPUT_IDS`` / ``_CURRENT_REGEN`` globals. Advances
    the per-sample counter exactly once per call; downstream extractors
    read the globals without further bookkeeping.
    """
...
def _ensure_lm_head(device: torch.device) -> None:
    ...
    Used by the logit-lens and actual-logprob blocks.
    """
...
def _logit_lens(layers: tuple[int, ...]) -> Callable:
    ...
    Per-layer dim = 9; total = 9 * len(layers).
    """
    @torch.no_grad()
    def extract(hidden_states, attention_mask, rmask):
...
def _length_features() -> Callable:
    ...
    def extract(hidden_states, attention_mask, rmask):
        ...
        return torch.tensor(
            [prompt_len, resp_len, seq_len, is_trunc], device=device,
        )
    return extract
...
def _cross_layer_mean_factory(
    ...
    """Mid-layer pool: per layer in ``layers``, take [last_token,
    mean(last K real tokens)]; stack the ``2 * n_layers`` vectors and
...
def _geo_features_factory() -> Callable:
    """101 hand-crafted geometric features:
        25 per-layer last-token L2 norms
        25 per-layer mean |activation| over response tokens
        24 layer-to-layer cosine sim at the last token
        24 layer-to-layer cosine sim at the response-token mean
         1 std of consecutive-token cosine sim within last layer's response
         1 sequence length (real tokens)
         1 response length
    """
    def extract(hidden_states, attention_mask, rmask):
        device = hidden_states.device
        ...
        if has_resp:
            ...
            if last_layer_resp.size(0) >= 2:
                tok_drift = F.cosine_similarity(
                    last_layer_resp[:-1], last_layer_resp[1:], dim=-1,
                )
        ...
        seq_len = torch.tensor(
            [float(attention_mask.sum().item())], device=device,
        )
        resp_len = torch.tensor(
            [float(rmask.sum().item())], device=device,
        )
...
def _actual_logprob_features_factory() -> Callable:
    ...
    The 9 within-layer features intentionally duplicate ``logit_lens_24`` —
    the tree-based sub-probe consuming this block will learn whichever
    representation it prefers.

    Reads ``_CURRENT_INPUT_IDS`` populated by ``response_mask()``.
    """
    @torch.no_grad()
    def extract(hidden_states, attention_mask, rmask):
        device = hidden_states.device
        ...
        within_9 = torch.stack([
            entropy.mean(), entropy.amin(), entropy.amax(),
            top1.mean(),    top1.amin(),    top1.amax(),
            margin.mean(),  margin.amin(),  margin.amax(),
        ])

        # 6 actual-token features at layer 24: predict response[t] from
        # hidden state at position t-1.
        ...
        if rmask_idx.numel() == 0:
            ...
            actual_6 = torch.stack([
                actual_lp.mean(), actual_lp.amin(), actual_lp.amax(),
                gap.mean(), gap.amax(), is_argmax.mean(),
            ])
...
def _regen_features_factory() -> Callable:
    """Read the per-sample 6-d regen-overlap row from the cache. The cache
    is populated by ``_ensure_regen_cache()`` before any block runs.
    """
    def extract(hidden_states, attention_mask, rmask):
        device = hidden_states.device
...
def _stat_features_factory(
    ...
    """Per-layer distribution-shape statistics of per-response-token L2
    norms. For each layer in ``layers``, compute 7 stats: mean, std, p25,
    p50, p75, excess kurtosis, entropy(softmax(norms)). Output dim =
    ``7 * len(layers)``.
    """
    def extract(hidden_states, attention_mask, rmask):
        device = hidden_states.device
        ...
        for L in layers:
            ...
            if n > 0:
                qs = torch.quantile(
                    norms, torch.tensor([0.25, 0.5, 0.75], device=device),
                )
...
def _layer_drift_l2_factory() -> Callable:
    """L2 magnitude of layer-to-layer change in hidden states, for each
    consecutive pair (L, L+1):
        ``||h[L+1, last_token] − h[L, last_token]||₂``  (24 scalars)
        ``||resp_mean[L+1] − resp_mean[L]||₂``          (24 scalars)
    Total 48 scalars on Qwen2.5-0.5B (25 hidden states → 24 pairs).
    """
    def extract(hidden_states, attention_mask, rmask):
        device = hidden_states.device
        last = int(attention_mask.nonzero(as_tuple=False)[-1].item())
...
def extract_geometric_features(
    ...
    # The geometric features are integrated via the BLOCKS registry
    # (geo_features, stat_features, layer_drift_l2). This stub exists only
    # to satisfy the public-surface contract.
...
#
# Building the cache here — before solution.py loads its own Qwen onto GPU —
# keeps memory sequential: this Qwen loads, generates, frees; then
# solution.py's Qwen loads. Late import of probe.SUB_PROBES lets us skip
# the build if no active sub-probe references regen_features.
# ---------------------------------------------------------------------------
